...
def plot_daily_profile_band(series, *, title: str, y_label: str):
    """
    series: length 8760 (assumed).
    Build daily matrix (365, 24), compute mean/min/max across days for each hour-of-day.
    """
    y = _to_1d_np(series)

    if len(y) != 8760:
        # fallback: if not 8760, do a simpler "hour-of-day" grouping by modulo
