# core/inputs/readers_timeseries.py
...
def read_csv_2level_timeseries(path: Path) -> xr.DataArray:
    ...
    # IMPORTANT: set index + NAME IT
    df.index = hour.astype(int).to_numpy()
    df.index.name = "hour"

    df = df.apply(pd.to_numeric, errors="coerce")

    s = df.stack(["scenario", "year"]).rename("value")   # index now includes "hour"
...
def read_csv_3level_timeseries(path: Path) -> xr.DataArray:
    """
    Read a CSV with a 3-row header: (scenario, year, resource)
    Meta column is expected to be (meta, hour, <empty>) but <empty> might be '' or NaN
    depending on how the CSV was edited/saved.
    Returns DataArray with dims: hour, scenario, year, resource
    """
    df = pd.read_csv(path, header=[0, 1, 2])
    df.columns = pd.MultiIndex.from_tuples(df.columns, names=["scenario", "year", "resource"])

    # --- robust meta/hour column detection ---
    meta_col = None
    for c in df.columns:
        # c is a tuple (scenario, year, resource)
